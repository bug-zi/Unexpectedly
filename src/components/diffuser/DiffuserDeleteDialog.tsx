...
interface DiffuserDeleteDialogProps {
  isOpen: boolean;
  word: string;
...
export function DiffuserDeleteDialog({
  isOpen,
  word,
  descendantCount,
  onConfirm,
  onCancel,
}: DiffuserDeleteDialogProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
        ...
        >
          ...
          >
            ...
            {/* 标题 */}
            <h3 className="text-center text-gray-700 dark:text-gray-200 font-medium text-base mb-2">确认删除</h3>

            {/* 内容 */}
            <p className="text-center text-gray-500 dark:text-gray-400 text-sm mb-1">
              确定要删除「<span className="text-gray-700 dark:text-white font-medium">{word}</span>」吗？
            </p>
            {descendantCount > 1 && (
              <motion.p
              ...
              >
                将同时删除 {descendantCount - 1} 个关联词语
              </motion.p>
            )}
            {descendantCount <= 1 && <div className="mb-4" />}
